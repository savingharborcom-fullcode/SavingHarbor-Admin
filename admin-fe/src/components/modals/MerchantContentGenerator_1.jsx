/**
 * SavingHarbor — Content Architecture Variation Engine
 * =====================================================
 * Pulls REAL data from your Supabase/Express backend,
 * combines it with a 384-combination structural variation
 * system, and generates genuinely unique content per merchant
 * via Gemini. No two pages will share the same structure,
 * tone, angle, heading style, OR underlying data.
 *
 * Setup:
 *  1. Set BACKEND_URL to your Render Express API base URL
 *  2. Ensure these Express endpoints exist (see bottom of file)
 *  3. Enter your Gemini API key in the UI
 *  4. Use Single mode to test, Batch mode for production
 */
...
// ─── VARIATION ENGINE ─────────────────────────────────────────────
// Deterministic hash — same merchant always gets the same variation
...
function getVariation(merchantName, category) {
  ...
  return {
    ...
    // Each section gets its own depth — creates length variation
...
async function fetchMerchantData(merchantSlug, backendUrl) {
  try {
    ...
    console.warn("DB fetch failed, proceeding without live data:", e.message);
    return null;
  }
}

// ─── PROMPT BUILDER ───────────────────────────────────────────────
function buildPrompt(merchantName, category, merchantUrl, variation, dbData) {
  const { blueprint, tone, angle, headingStyle, sectionDepths } = variation;
  const now = new Date();
  const month = now.toLocaleString("default", { month: "long" });
  const year = now.getFullYear();

  // Format real DB data for the prompt
  let dbSection = "";
  if (dbData) {
    const topCoupons = (dbData.coupons || []).slice(0, 8);
    const discountSummary = buildDiscountSummary(topCoupons);
    dbSection = `
REAL DATA FROM DATABASE (use these specific facts — do NOT invent):
- Merchant: ${merchantName}
- Category: ${dbData.category || category}
- Total active coupons: ${dbData.totalCoupons || "unknown"}
- Active deals: ${dbData.totalDeals || "unknown"}
${discountSummary}
${dbData.avgDiscount ? `- Average discount: ${dbData.avgDiscount}%` : ""}
${dbData.maxDiscount ? `- Best discount available: ${dbData.maxDiscount}%` : ""}
${dbData.hasFreeShipping ? "- Free shipping offer available" : ""}
${dbData.hasNewUserOffer ? "- New customer/first order offer available" : ""}
${dbData.couponTypes ? `- Coupon types: ${dbData.couponTypes.join(", ")}` : ""}
${dbData.lastUpdated ? `- Offers last verified: ${dbData.lastUpdated}` : ""}

TOP OFFERS TO REFERENCE (cite these specifically in the content):
${topCoupons
  .map((c, i) => `  ${i + 1}. "${c.title}" — ${formatDiscount(c)} ${c.code ? `[code: ${c.code}]` : "(no code needed)"}`)
  .join("\n")}
`;
  }

  // Section instructions with dynamic depth and headings
  const sectionInstructions = blueprint.sections
    .map((s, i) => {
      ...
      const wordRange =
        depth === "brief" ? "55-80" : depth === "standard" ? "90-120" : "130-170";
      return `  "${s.id}": {
    "heading": "${heading}",
    "body": "[WRITE ${wordRange} words here. ${tone.instruction} ${angle.instruction} Must reference ${merchantName} specifically. ${dbData && i === 0 ? `Mention the ${dbData.totalCoupons || "many"} active coupons available.` : ""} ${dbData && i === blueprint.sections.length - 1 ? `Reference the best available discount (${dbData.maxDiscount || "up to significant"}% off).` : ""}]"
  }`;
    })
    .join(",\n");

  return `You are a senior SEO content writer for SavingHarbor.com, a verified coupon deals platform.

MERCHANT: "${merchantName}"
CATEGORY: "${category}"
${merchantUrl ? `WEBSITE: ${merchantUrl}` : ""}
CONTENT DATE: ${month} ${year}

WRITING PROFILE:
- Tone: ${tone.instruction}
- Content angle: ${angle.instruction}
- Blueprint: ${blueprint.label}

${dbSection}

ABSOLUTE RULES — breaking these makes the output useless:
1. NEVER use these exact phrases (they trigger Google's HCS penalty):
   - "combining discounts…unlock even bigger savings"
   - "turns a normal purchase into a smarter one"
   - "strategic shopping" / "budget-focused shoppers"
   - "during these periods" / "savings-focused"
   - "plan, compare, and use" / "smarter financial decision"
2. Every section must contain facts SPECIFIC to ${merchantName} — not generic category filler
3. If real coupon data is provided above, reference specific offers, discount values, and codes
4. Vary sentence structure between sections — no repetitive patterns
5. Write as a knowledgeable human, not a content template
6. Total word count across all sections: 600-780 words

Return ONLY valid JSON — no markdown fences, no comments, no explanation:

{
  "seoTitle": "${merchantName} Coupons & Promo Codes (${month} ${year}) | Verified | SavingHarbor",
  "metaDescription": "[Under 155 chars. Specific to ${merchantName}. Include ${category} and mention ${dbData?.maxDiscount ? `up to ${dbData.maxDiscount}% off` : "verified savings"}. Natural CTA.]",
  "h1Tag": "[Unique phrasing, different from title, under 68 chars]",
  "focusKeywords": [
    "${merchantName} coupons",
    "${merchantName} promo codes",
    "${merchantName} discount code",
    "${merchantName} coupon code",
    "${merchantName} deals",
    "${merchantName} offers",
    "${merchantName} voucher"
  ],
  "lsiKeywords": [
    "verified ${merchantName} codes",
    "${merchantName} sale",
    "${merchantName} discount",
    "${merchantName} cashback",
    "${merchantName} free shipping",
    "${merchantName} student discount",
    "${merchantName} first order discount",
    "save at ${merchantName}",
    "${merchantName} checkout code",
    "${merchantName} ${category} coupons",
    "best ${merchantName} deals",
    "${merchantName} clearance",
    "${merchantName} newsletter discount",
    "${merchantName} loyalty rewards",
    "${merchantName} exclusive offer"
  ],
  "sections": {
${sectionInstructions}
  },
  "faqItems": [
    {
      "question": "[Specific FAQ about ${merchantName} — not generic]",
      "answer": "[Specific answer. ${dbData ? `Reference actual discount data where relevant.` : ""}]"
    },
    {
      "question": "[Specific FAQ about ${merchantName} checkout or codes]",
      "answer": "[Specific step-by-step or policy answer for ${merchantName}]"
    },
    {
      "question": "[FAQ about ${merchantName} shipping, returns, or eligibility]",
      "answer": "[Specific ${merchantName} policy details]"
    },
    {
      "question": "[FAQ about ${merchantName} seasonal deals or best time to buy]",
      "answer": "[Specific ${merchantName} sale calendar or timing advice]"
    },
    {
      "question": "[FAQ about stacking, combining, or limitations at ${merchantName}]",
      "answer": "[Specific answer about ${merchantName}'s coupon policies]"
    }
  ],
  "schemaData": {
    "breadcrumbName": "${merchantName}",
    "pageDescription": "[One sentence, under 120 chars, for schema]"
  },
  "variationProfile": "${blueprint.label} | ${tone.label} | ${angle.label} | ${headingStyle.label} headings"
}`;
}

function buildDiscountSummary(coupons) {
  if (!coupons.length) return "";
  const pct = coupons.filter((c) => c.discountType === "percentage" || c.type === "percentage");
  const flat = coupons.filter((c) => c.discountType === "flat" || c.type === "flat");
  const free = coupons.filter((c) => c.discountType === "free_shipping" || c.type === "free_shipping");
  const bogo = coupons.filter((c) => c.discountType === "bogo" || c.type === "bogo");
  let lines = [];
  if (pct.length) lines.push(`- Percentage-off coupons: ${pct.length} (e.g. ${pct.slice(0, 2).map((c) => formatDiscount(c)).join(", ")})`);
  if (flat.length) lines.push(`- Flat-amount discounts: ${flat.length} (e.g. ${flat.slice(0, 2).map((c) => formatDiscount(c)).join(", ")})`);
  if (free.length) lines.push(`- Free shipping offers: ${free.length}`);
  if (bogo.length) lines.push(`- BOGO/bundle deals: ${bogo.length}`);
  return lines.join("\n");
}
...
function formatDiscount(coupon) {
  ...
  const val = coupon.discountValue || coupon.value || coupon.discount_value || "";
  if (type === "percentage" && val) return `${val}% off`;
  if (type === "flat" && val) return `$${val} off`;
  if (type === "free_shipping") return "free shipping";
  if (type === "bogo") return "buy one get one";
  return val ? `${val} off` : "special offer";
}
...
function CopyBtn({ text, label = "Copy" }) {
  ...
  return (
    <button onClick={copy} style={{
      ...
      background: copied ? "var(--color-background-secondary)" : "var(--color-background-primary)",
  ...
  return (
    <div style={{ marginBottom: "0.9rem" }}>
      ...
      <div style={{
        ...
        borderRadius: 6, padding: "8px 12px", fontSize: 13, lineHeight: 1.65,
...
function Section({ s }) {
  ...
  return (
    <div style={{ border: "0.5px solid var(--color-border-tertiary)", borderRadius: 8, marginBottom: 8, overflow: "hidden" }}>
      ...
      {open && (
        <div style={{ padding: "10px 12px", fontSize: 13, lineHeight: 1.78, color: "var(--color-text-primary)" }}>
          {body}
        </div>
      )}
    </div>
...
function DBStatus({ status }) {
  const styles = {
    connected: { bg: "#EAF3DE", color: "#2E5C0E", icon: "✓", text: "DB data loaded" },
    failed: { bg: "#FAEEDA", color: "#854F0B", icon: "⚠", text: "Using Gemini only (no DB data)" },
    loading: { bg: "#E6F1FB", color: "#185FA5", icon: "⟳", text: "Fetching merchant data…" },
    idle: { bg: "var(--color-background-secondary)", color: "var(--color-text-secondary)", icon: "○", text: "DB not fetched yet" },
  };
...
export default function VariationEngine() {
  ...
  const runSingle = async () => {
    ...
    if (!merchant || !category) { setError("Enter merchant name and category."); return; }
    setError(""); setOutput(null); setRunning(true);

    let dbData = null;
    if (useDB && merchantSlug) {
      setDbStatus("loading");
      setStatus("Fetching merchant data from DB…");
    ...
    try {
      ...
      const raw = await callGemini(buildPrompt(merchant, category, url, variation, dbData), apiKey, model);
      const data = safeJSON(raw);
      setOutput({ ...data, variation, dbData });
      setTab("seo");
    } catch (e) {
      setError(e.message);
    }
  ...
  const runBatch = async () => {
    ...
    setError(""); setBatchResults([]); stopRef.current = false; setRunning(true);
    ...
    for (let i = 0; i < rows.length; i++) {
      ...
      try {
        ...
        const raw = await callGemini(buildPrompt(r.name, r.category, r.url, variation, dbData), apiKey, model);
        const data = safeJSON(raw);
        setBatchResults((prev) => [...prev, {
          merchant: r.name, category: r.category, slug: r.slug,
          status: "done", ...data, variation, dbData,
        }]);
      } catch (e) {
        setBatchResults((prev) => [...prev, {
          merchant: r.name, category: r.category, status: "error", error: e.message,
        }]);
      }
      if (i < rows.length - 1) await new Promise((res) => setTimeout(res, 1500));
    }
    setRunning(false);
    setStatus(stopRef.current ? "Stopped." : `Done — ${batchResults.length} merchants processed.`);
  };

  const exportCSV = (results) => {
    const done = results.filter((r) => r.status === "done");
    const headers = ["merchant", "category", "slug", "variation_profile", "seo_title", "meta_description", "h1_tag", "focus_keywords", "total_coupons", "max_discount"];
    const rows = done.map((r) => [
      r.merchant, r.category, r.slug || "",
    ...
    a.download = `savingharbor-content-${Date.now()}.csv`; a.click();
  };

  const exportJSON = (results) => {
    const blob = new Blob([JSON.stringify(results, null, 2)], { type: "application/json" });
    const a = document.createElement("a"); a.href = URL.createObjectURL(blob);
    a.download = `savingharbor-content-${Date.now()}.json`; a.click();
  };
  ...
  const estCost = (n) => (n * (model.includes("flash") ? 0.002 : 0.018)).toFixed(2);
  ...
  const tabStyle = (active) => ({
    padding: "6px 13px", fontSize: 12, fontFamily: "inherit", cursor: "pointer",
    border: "0.5px solid var(--color-border-secondary)", borderRadius: 6,
  ...
  return (
    <div style={{ padding: "1.5rem 0", fontFamily: "var(--font-sans)", color: "var(--color-text-primary)", maxWidth: 700 }}>

      {/* ── HEADER ── */}
      <div style={{ background: "#0F2240", borderRadius: 12, padding: "1.1rem 1.4rem", marginBottom: "1.25rem" }}>
        <div style={{ fontSize: 16, fontWeight: 600, color: "#fff", marginBottom: 4 }}>
          ⚡ Content Architecture Variation Engine
        </div>
        <div style={{ fontSize: 12, color: "#7AAAC8", lineHeight: 1.65 }}>
          SavingHarbor · DB-powered · 384 structural combinations · Gemini content generation
        </div>
        <div style={{ display: "flex", gap: 8, marginTop: 10, flexWrap: "wrap" }}>
          {["8 blueprints", "4 tones", "4 angles", "3 heading styles", "Real coupon data", "Batch export"].map((t) => (
            <span key={t} style={{ fontSize: 11, padding: "2px 8px", background: "rgba(255,255,255,.1)", color: "#9BB8D4", borderRadius: 4 }}>{t}</span>
          ))}
        </div>
      </div>
      ...
      <div style={{ background: "var(--color-background-secondary)", border: "0.5px solid var(--color-border-tertiary)", borderRadius: 10, padding: "1rem", marginBottom: "1rem" }}>
        ...
        <div style={{ display: "grid", gridTemplateColumns: "1fr auto", gap: 10, alignItems: "end" }}>
          <div>
            <label style={{ fontSize: 12, fontWeight: 500, color: "var(--color-text-secondary)", display: "block", marginBottom: 4 }}>Express Backend URL (for DB data)</label>
          ...
          <div style={{ paddingBottom: 1 }}>
            ...
            <div
              ...
              style={{
                ...
                border: "0.5px solid var(--color-border-secondary)", transition: "background .2s",
              }}
            >
              <div style={{
                position: "absolute", top: 3, left: useDB ? 22 : 3,
        ...
        <div>
          <div style={{ background: "var(--color-background-primary)", border: "0.5px solid var(--color-border-tertiary)", borderRadius: 10, padding: "1rem", marginBottom: "1rem" }}>
            ...
            <div style={{ display: "grid", gridTemplateColumns: "1fr 1fr", gap: 10 }}>
              ...
              <div>
                <label style={{ fontSize: 12, fontWeight: 500, color: "var(--color-text-secondary)", display: "block", marginBottom: 4 }}>DB Slug (for live coupon data)</label>
            ...
            <div style={{ background: "var(--color-background-secondary)", border: "0.5px solid var(--color-border-tertiary)", borderRadius: 8, padding: "0.9rem 1rem", marginBottom: "0.9rem", fontSize: 13 }}>
              ...
              {preview.blueprint.sections.map((s, i) => (
                <div key={i} style={{ fontSize: 12, color: "var(--color-text-secondary)", paddingLeft: 8 }}>
                  ...
                  <span style={{ marginLeft: 6, fontSize: 10, opacity: .6 }}>({preview.sectionDepths[i]})</span>
                </div>
              ))}
            </div>
          )}
          ...
          <div style={{ display: "flex", gap: 8, marginBottom: "1rem" }}>
            ...
            <button onClick={runSingle} disabled={running} style={{
              flex: 1, padding: "9px", fontSize: 14, fontWeight: 500, border: "none", borderRadius: 8,
              background: running ? "var(--color-background-tertiary)" : "#0F2240",
              color: running ? "var(--color-text-tertiary)" : "#fff", cursor: running ? "not-allowed" : "pointer", fontFamily: "inherit",
            }}>
                ...
                <div style={{ fontSize: 13 }}>
                  {output.dbData ? (
                    ...
                    <div style={{ background: "#FAEEDA", border: "0.5px solid #EF9F27", borderRadius: 6, padding: "8px 12px", color: "#854F0B" }}>
                      ⚠ No DB data was used. Either useDB is off, no slug was provided, or the backend call failed. Content was generated from Gemini's training data only.
                    </div>
                  )}
                </div>
              )}
      ...
      {mode === "batch" && (
        <div>
          <div style={{ background: "#FAEEDA", border: "0.5px solid #EF9F27", borderRadius: 8, padding: "8px 12px", marginBottom: "0.9rem", fontSize: 12, color: "#854F0B", lineHeight: 1.7 }}>
            <strong>CSV Format:</strong> <code style={{ fontFamily: "var(--font-mono)", background: "#FFF3CD", padding: "1px 5px", borderRadius: 3 }}>Merchant Name, Category, Website URL, DB Slug</code>
            <br />Website and Slug are optional but Slug enables real coupon data from your DB.
            <br />Example: <code style={{ fontFamily: "var(--font-mono)", background: "#FFF3CD", padding: "1px 5px", borderRadius: 3 }}>Healthyline, Health & Wellness, https://healthyline.com, healthyline-coupons</code>
          </div>
          ...
          <div style={{ display: "flex", gap: 8, marginBottom: "1rem" }}>
            <button onClick={runBatch} disabled={running || !batchRows.length} style={{
              flex: 1, padding: "9px", fontSize: 14, fontWeight: 500, border: "none", borderRadius: 8,
              background: running ? "var(--color-background-tertiary)" : "#0F2240",
              color: running ? "var(--color-text-tertiary)" : "#fff", cursor: running ? "not-allowed" : "pointer", fontFamily: "inherit",
            }}>
            ...
            <div>
              <div style={{ display: "flex", justifyContent: "space-between", alignItems: "center", marginBottom: 8 }}>
                <span style={{ fontSize: 13, fontWeight: 500 }}>
                  {batchResults.filter((r) => r.status === "done").length} done · {batchResults.filter((r) => r.status === "error").length} errors
                </span>
                <div style={{ display: "flex", gap: 6 }}>
                  ...
                  <button onClick={() => exportJSON(batchResults)} style={{ fontSize: 11, padding: "3px 10px", border: "0.5px solid var(--color-border-secondary)", borderRadius: 4, background: "var(--color-background-primary)", cursor: "pointer", fontFamily: "inherit" }}>⬇ Full JSON</button>
                </div>
              </div>
              <div style={{ maxHeight: 320, overflowY: "auto", border: "0.5px solid var(--color-border-tertiary)", borderRadius: 8 }}>
                {batchResults.map((r, i) => (
                  <div key={i} style={{
                    ...
                    {r.status === "done" && <span style={{ fontSize: 10, padding: "1px 6px", background: "#EAF3DE", color: "#2E5C0E", borderRadius: 3 }}>done</span>}
                  </div>
                ))}
              </div>
            </div>
