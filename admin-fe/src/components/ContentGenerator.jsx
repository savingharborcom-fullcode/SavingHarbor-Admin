...
function formatDiscount(c) {
  if (!c) return null;
  if (c.discountType === "percentage" && c.value) return `${c.value}% off`;
  if (c.discountType === "flat" && c.value) return `$${c.value} off`;
...
export default function VariationEngine() {
  ...
  const runBatch = async () => {
    ...
    setError(""); setBatchResults([]); stopRef.current = false;
    ...
    for (let i = 0; i < rows.length; i++) {
      ...
      try {
        const research = safeJSON(await callGemini(buildResearchPrompt(r.name, r.category, r.url, crawledText, dbData), apiKey, model));
        const variation = getVariation(r.name, r.category);
        const data = safeJSON(await callGemini(buildFinalPrompt(r.name, r.category, research, variation, dbData, r.url), apiKey, model));
        ...
        setBatchResults((prev) => [...prev, { merchant: r.name, category: r.category, slug: r.slug, status: "done", saved: savedOk, ...data, variation, dbData }]);
      } catch (e) {
        setBatchResults((prev) => [...prev, { merchant: r.name, category: r.category, status: "error", error: e.message }]);
      }
        ...
        <div style={{ display: "grid", gridTemplateColumns: "1fr 1fr", gap: 10, marginBottom: 10 }}>
          ...
          <div>
            ...
            <select value={model} onChange={(e) => setModel(e.target.value)} style={{ ...inputStyle, height: 36 }} disabled={running}>
              <option value="gemini-2.5-flash-lite">gemini-2.5-flash-lite (most affordable)</option>
      ...
      {mode === "batch" && (
        <div>
          <div style={{ background: "#FAEEDA", border: "0.5px solid #EF9F27", borderRadius: 8, padding: "8px 12px", marginBottom: "0.9rem", fontSize: 12, color: "#854F0B", lineHeight: 1.7 }}>
            ...
            <br />
            Slug enables DB coupon fetch AND auto-saves generated content back to the merchants table.
          </div>

          <div style={{ marginBottom: "0.9rem" }}>
            <label style={{ fontSize: 12, fontWeight: 500, color: "var(--color-text-secondary)", display: "block", marginBottom: 4 }}>Merchant List (up to 500 per session)</label>
          ...
          {batchRows.length > 0 && (
            <div style={{ background: "var(--color-background-secondary)", border: "0.5px solid var(--color-border-tertiary)", borderRadius: 8, padding: "8px 12px", marginBottom: "0.9rem", fontSize: 13, display: "flex", gap: 20, flexWrap: "wrap" }}>
              ...
              <span>⏱ ~{Math.ceil((batchRows.length * 5) / 60)} min</span>
            ...
            <div>
              <div style={{ display: "flex", justifyContent: "space-between", alignItems: "center", marginBottom: 8 }}>
                <span style={{ fontSize: 13, fontWeight: 500 }}>
                  {batchResults.filter((r) => r.status === "done").length} done · {batchResults.filter((r) => r.status === "error").length} errors · {batchResults.filter((r) => r.saved).length} saved to DB
                </span>
              ...
              <div style={{ maxHeight: 320, overflowY: "auto", border: "0.5px solid var(--color-border-tertiary)", borderRadius: 8 }}>
                {batchResults.map((r, i) => (
                  <div key={i} style={{ padding: "8px 12px", borderBottom: "0.5px solid var(--color-border-tertiary)", display: "flex", alignItems: "center", gap: 10, fontSize: 13, background: i % 2 === 0 ? "var(--color-background-primary)" : "var(--color-background-secondary)" }}>
